...
export async function loadStudentReport(profileId, period='90d') {
  ...
  const totalMeters=activities.reduce((s,a)=>s+num(a.distancia_metros),0)
  const movingSeconds=activities.reduce((s,a)=>s+num(a.tiempo_movimiento_segundos),0)
  const elevation=activities.reduce((s,a)=>s+num(a.desnivel_metros),0)
  const calories=activities.reduce((s,a)=>s+num(a.calorias),0)
  const weightedSpeed=activities.reduce((s,a)=>s+num(a.velocidad_media_ms)*num(a.tiempo_movimiento_segundos),0)
  const maxSpeed=Math.max(0,...activities.map(a=>num(a.velocidad_maxima_ms)))
  const completed=safe(resultsR.data).filter(r=>r.status==='completed'||r.completed_at)
  const reservations=safe(reservationsR.data)
  return {
    generatedAt:new Date().toISOString(), period, profile:profileR.data, performance:performanceR.data,
    summary:{activities:activities.length,km:totalMeters/1000,movingSeconds,elevationMeters:elevation,calories:calories||null,avgSpeedKmh:movingSeconds?weightedSpeed/movingSeconds*3.6:null,maxSpeedKmh:maxSpeed?maxSpeed*3.6:null,tasksCompleted:completed.length,reservations:reservations.length,classesDone:reservations.filter(r=>r.estado==='realizada').length},
    activities, notes:safe(notesR.data), goals:safe(goalsR.data), takes:safe(takesR.data), trainingResults:safe(resultsR.data), reservations
  }
}
...
export function reportSheets(report){
 ...
 return {
  Resumen:[['Campo','Valor'],['Alumno',[p.nombre,p.apellido].filter(Boolean).join(' ')],['Período',report.period],['Actividades inline',s.activities],['Kilómetros',s.km],['Tiempo movimiento (s)',s.movingSeconds],['Desnivel (m)',s.elevationMeters],['Velocidad media (km/h)',s.avgSpeedKmh??'No disponible'],['Velocidad máxima (km/h)',s.maxSpeedKmh??'No disponible'],['Calorías',s.calories??'No disponible'],['Deberes completados',s.tasksCompleted],['Clases realizadas',s.classesDone]],
