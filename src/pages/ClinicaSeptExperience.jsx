      ...
      <div className="relative mx-auto w-full max-w-7xl px-4 pb-16 pt-5 sm:px-6 lg:px-8">
        ...
        <section className="relative mt-5 overflow-hidden rounded-[38px] border border-white/[.08] bg-[linear-gradient(135deg,#121214_0%,#17100d_58%,#2a1208_100%)] shadow-[0_35px_110px_rgba(0,0,0,.38)]">
          ...
          <div className="relative grid gap-8 p-6 sm:p-9 lg:grid-cols-[1.18fr_.82fr] lg:p-12">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-[#FF8A2A]/30 bg-[#FF6A1A]/10 px-3 py-1.5 text-[9px] font-black uppercase tracking-[.18em] text-[#FFB13B]">EXPERIENCIA COMPLETADA · 2026</div>
              <h1 className="mt-5 max-w-4xl text-[46px] font-black leading-[.86] tracking-[-.06em] sm:text-6xl lg:text-[78px]">PRIMERA CLÍNICA<br />INTERNACIONAL <span className="bg-gradient-to-r from-[#FF8A2A] via-[#FF5E1A] to-[#FFC857] bg-clip-text text-transparent">DE PATINAJE</span></h1>
              <p className="mt-5 max-w-2xl text-sm font-medium leading-6 text-white/58 sm:text-base">Tres jornadas de formación intensiva en Punta del Este junto a Miguel Flores, Subcampeón Máster Mundial y entrenador con más de 40 años de experiencia.</p>
            ...
            <div className="mt-7 grid gap-3 md:grid-cols-2">
              {CONTENT.map((item) => (
                <div key={item.label} className="rounded-[22px] border border-white/[.07] bg-[#0f0f11] p-4 shadow-[0_10px_28px_rgba(0,0,0,.16)]">
                  <div className="flex items-center justify-between gap-3"><p className="text-xs font-black text-white/74">{item.label}</p><span className="rounded-full bg-white/[.05] px-2.5 py-1 text-[9px] font-black uppercase tracking-[.08em] text-white/42">{item.level}</span></div>
                  <div className="mt-3 h-2.5 overflow-hidden rounded-full bg-white/[.06]"><div className={`h-full rounded-full bg-gradient-to-r ${item.tone}`} style={{ width: item.width }} /></div>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="pt-10">
          <div className="relative overflow-hidden rounded-[34px] border border-[#FF8A2A]/18 bg-[linear-gradient(135deg,#17100d_0%,#111113_55%,#251208_100%)] p-6 shadow-[0_24px_80px_rgba(0,0,0,.22)] sm:p-8">
            ...
            <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex items-start gap-4"><div className="grid h-16 w-16 shrink-0 place-items-center rounded-[20px] border border-[#FFB13B]/25 bg-[#FF6A1A]/12 text-3xl shadow-[0_10px_26px_rgba(255,106,26,.12)]">🏅</div><div><p className="text-[10px] font-black uppercase tracking-[.2em] text-[#FFB13B]">INSIGNIA OFICIAL PR</p><h2 className="mt-2 text-3xl font-black tracking-[-.045em]">Si completaste los 3 días, tu insignia ya forma parte de tu perfil.</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-white/52">La insignia “Primera Clínica 2026 realizada” queda cargada en tu perfil Punta Rollers como reconocimiento por haber completado la experiencia.</p></div></div>
        ...
        <footer className="mt-12 border-t border-white/[.08] py-8 text-center"><p className="text-[10px] font-black uppercase tracking-[.22em] text-white/28">PUNTA ROLLERS · PUNTA DEL ESTE · URUGUAY</p><p className="mt-2 text-sm font-black text-[#FF8A2A]">No es solo patinar. Es pertenecer.</p></footer>
      </div>
    </main>
  )
}
