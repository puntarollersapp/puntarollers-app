...
const VISIBILITY = [{ key: 'all', label: 'Todos PR' }, { key: 'friends', label: 'Solo amigos' }, { key: 'self', label: 'Solo yo' }]
...
export default function PRMoments() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from('pr_moment_comments').select('*').eq('moment_id', selected.id).is('deleted_at', null).order('created_at'),
  ...
  async function createMoment(event) {
    ...
    try {
      ...
      const { error } = await supabase.from('pr_moments').insert({ profile_id: currentId, media_type: kind, caption: cleanCaption || null, storage_path: storagePath, visibility })
      if (error) throw error
  ...
  async function postComment(event) {
    ...
    if (error) setMessage(error.message)
    else { setComment(''); const { data } = await supabase.from('pr_moment_comments').select('*').eq('moment_id', selected.id).is('deleted_at', null).order('created_at'); setComments(data || []) }
    setSaving(false)
  }

  async function removeMoment() {
    if (!selected || !window.confirm('¿Eliminar este Moment?')) return
    const { error } = await supabase.from('pr_moments').update({ deleted_at: new Date().toISOString(), deleted_by_profile_id: currentId }).eq('id', selected.id)
    if (error) return setMessage(error.message)
    closeViewer(); await load()
  }

  async function removeComment(id) {
    const { error } = await supabase.from('pr_moment_comments').update({ deleted_at: new Date().toISOString(), deleted_by_profile_id: currentId }).eq('id', id)
