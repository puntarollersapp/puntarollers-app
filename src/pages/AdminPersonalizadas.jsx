...
export default function AdminPersonalizadas() {
  ...
  const addSlot = async (e) => {
    ...
    setBusy(true)
    const { error } = await supabase.from('pr_personal_disponibilidad').insert({ ...slot, habilitado: true })
    setBusy(false)
    if (error) return setNotice(error.message)
    ...
    <div className="min-h-screen bg-[#080808] px-4 pb-24 pt-6 text-white">
      <div className="mx-auto max-w-5xl space-y-6">
        <header className="flex items-start justify-between gap-4">
          <div><Link to="/admin" className="text-xs font-bold text-white/35">← Volver al admin</Link><p className="mt-4 text-[10px] font-black uppercase tracking-[.34em] text-red-300">PUNTA ROLLERS · ADMIN</p><h1 className="mt-1 text-3xl font-black">PR Personal</h1></div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <section className="rounded-[28px] border border-white/10 bg-white/[.035] p-5"><p className="text-[10px] font-black uppercase tracking-[.25em] text-white/35">Agenda semanal</p><h2 className="mt-1 text-xl font-black">Agregar horario</h2><form onSubmit={addSlot} className="mt-4 grid grid-cols-2 gap-3"><input type="date" value={slot.fecha} onChange={(e) => setSlot({ ...slot, fecha: e.target.value })} className="col-span-2 rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm" /><input type="time" value={slot.hora_inicio} onChange={(e) => setSlot({ ...slot, hora_inicio: e.target.value })} className="rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm" /><input type="time" value={slot.hora_fin} onChange={(e) => setSlot({ ...slot, hora_fin: e.target.value })} className="rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm" /><button disabled={busy} className="col-span-2 rounded-2xl bg-red-500 px-4 py-3 text-sm font-black">Publicar turno</button></form><div className="mt-4 max-h-64 space-y-2 overflow-auto">{futureSlots.map((x) => <div key={x.id} className="flex items-center justify-between rounded-2xl border border-white/10 bg-black/20 p-3"><div><p className="text-sm font-black">{day(x.fecha)}</p><p className="text-xs text-white/40">{hhmm(x.hora_inicio)}–{hhmm(x.hora_fin)}</p></div><button onClick={async () => { await supabase.from('pr_personal_disponibilidad').update({ habilitado: false }).eq('id', x.id); load() }} className="text-xs font-bold text-red-300">Ocultar</button></div>)}</div></section>
        </div>

        <section className="rounded-[28px] border border-white/10 bg-white/[.035] p-5"><p className="text-[10px] font-black uppercase tracking-[.25em] text-white/35">Clases</p><h2 className="mt-1 text-xl font-black">Reservas recientes</h2><div className="mt-4 space-y-3">{reservations.length === 0 && <p className="text-sm text-white/35">Todavía no hay reservas.</p>}{reservations.map((r) => { const s = slotMap[r.disponibilidad_id]; const p = profiles[r.alumno_id]; return <article key={r.id} className="rounded-2xl border border-white/10 bg-black/25 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-black">{p?.nombre || 'Alumno'} {p?.apellido || ''}</p><p className="mt-1 text-xs text-white/40">{s ? `${day(s.fecha)} · ${hhmm(s.hora_inicio)}–${hhmm(s.hora_fin)}` : 'Turno'}</p></div><span className="rounded-full border border-white/10 px-3 py-1 text-[10px] font-black uppercase">{r.estado}</span></div>{r.estado === 'reservada' && <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4"><button onClick={() => status(r, 'realizada')} className="rounded-xl bg-emerald-500/15 px-3 py-2 text-xs font-black text-emerald-200">✓ Realizada</button><button onClick={() => status(r, 'suspendida', 'Lluvia / mal clima')} className="rounded-xl bg-sky-500/15 px-3 py-2 text-xs font-black text-sky-200">☔ Clima</button><button onClick={() => status(r, 'suspendida', 'Imprevisto')} className="rounded-xl bg-amber-500/15 px-3 py-2 text-xs font-black text-amber-100">Imprevisto</button><button onClick={() => status(r, 'cancelada', 'Aviso con tiempo')} className="rounded-xl bg-white/[.06] px-3 py-2 text-xs font-black text-white/65">Cancelar</button></div>}</article>})}</div></section>
